        ...
        while result == "" and field.empty_cells():
            ...
            try:
                ...
                if turns[turn] == 0:
                    ...
                    print(
                        bcolors.BOLD + f"{players[turns[turn]].name}: " + bcolors.ENDC
                    )
                    choice = players[turns[turn]].choose_cell(
                        field.empty_cells(),
                ...
                if choice == "stop":
                    result = (
                        bcolors.OKCYAN
                        + f"{players[turns[turn]].name} have interrupted the game."
                        + bcolors.ENDC
                    )

                elif (
                    choice not in players[turns[turn]].choices
                    and choice not in players[turns[turn + 1]].choices
                ):
                    ...
                    print(
                        bcolors.WARNING + "Please, choose an empty cell." + bcolors.ENDC
                    )
                    continue
                ...
                print(
                    bcolors.WARNING + "Type in one of the digits:",
                    str(empty_cells) + bcolors.ENDC,
                )
                continue
